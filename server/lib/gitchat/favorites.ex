defmodule Gitchat.Favorites do
  @moduledoc """
  ...
  def get_by_user(user_id) do
    query = from r in Favorite, where: r.user_id == ^user_id, order_by: [desc: :inserted_at]
    Repo.all(query)
  end

  def get_by_user_repo(user_id, repo) do
    query = from r in Favorite, where: r.user_id == ^user_id and r.repo == ^repo
    Repo.one(query)
  end
end
